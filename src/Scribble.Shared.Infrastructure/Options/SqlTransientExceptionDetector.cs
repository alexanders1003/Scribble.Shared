...
public class SqlTransientExceptionDetector : IExceptionDetector
{
    ...
    public bool ShouldRetryRequestOn(Exception exp)
    {
        if (exp is not SqlException sqlException)
            return exp is TimeoutException;
        
        return (from SqlError error in sqlException.Errors 
            select HandledErrorNumbers.Contains(error.Number)).FirstOrDefault();
    }
}